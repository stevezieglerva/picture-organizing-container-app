...
def generate_html_table(grid, image_path: str, count: int):
    html = f"<h2>{image_path}</h2><img width=300px; src='{image_path}'/><img src='lawn_histogram_{count}.jpg'/><br><table rules='none'>"
    for row in grid:
        html += "<tr>"
        for color in row:
            # color_name, matched_color = classify_color(color)
            html += (
                f'<td height=5px; width=5px; style="background-color: {color};"></td>'
            )
        html += "</tr>"
    html += "</table>"
    return html
...
def analyze_lawn(count: int, image_path: str, width: int, grouping: int):
    # Load the image
    picture = Picture(image_path, ImageIOLocal())
    print(picture)
    print(picture.hash_average_hash)
    image = picture._pil_image
    print(image.size)
    ...
    html_table = generate_html_table(rotated_grid, image_path, count)
    ...
    plt.savefig(f"lawn_histogram_{count}.jpg")
    return html_table, picture.hash_phash


# Usage example
images = sorted(glob("2023*.jpg"))  # + sorted(glob("2023*.jpeg"))
html = ""
hashes = {}
for count, image in enumerate(images):
    print(f"\n\nProcessing: #{count+1}: {image}")
    more_html, phash = analyze_lawn(count, image, 100, 100)
    html += more_html
    hashes[image] = str(phash)
...
for k, v in hashes.items():
    ...
    print(f"Checking: '{image}' with {hex_hash}")
    for tk, tv in hashes.items():
        test_image = tk
        if test_image == image:
            continue
        test_hex_hash = tv
        test_hash = imagehash.hex_to_hash(test_hex_hash)
        delta = hash - test_hash
        print(f"\t'{test_image}': {delta}")
        if delta <= 15:
            print(f"Similar!\n{image}\n{test_image}")
